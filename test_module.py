...
def test_flask_GST001():
    ...
    assert result.status_code == 200
    assert {'id': 0, 'duration': 3.0} == json.loads(result.data)
    result = client.get("/get_speaking_time", json={"id": 1})
    assert result.status_code == 200
    assert {'id': 1, 'duration': 2.0} == json.loads(result.data)
    result = client.get("/get_speaking_time", json={"id": 2})
    assert result.status_code == 200
    assert {'id': 2, 'duration': 4.0} == json.loads(result.data)
    result = client.get("/get_speaking_time", json={"id": 3})
    assert result.status_code == 200
    assert {'id': 3, 'duration': 5.0} == json.loads(result.data)
    result = client.get("/get_speaking_time", json={"id": 4})
    assert result.status_code == 200
    assert {'id': 4, 'duration': 6.0} == json.loads(result.data)
